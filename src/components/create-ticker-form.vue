<template>
  <form @submit.prevent="createTicker">
    <div class="'flex">
      <div class="max-w-xs">
        ...
        <div class="mt-1 relative rounded-md shadow-md">
          <input
            v-model="ticker"
            @input="$emit('change-ticker', $event)"
            type="text"
            name="wallet"
            id="wallet"
...
export default {
  ...
  emits: {
    'create-ticker': (value) => typeof value === 'string',
    'change-ticker': null,
  },
  ...
  methods: {
    createTicker() {
      if (!this.ticker.trim().length) {
        return;
      }

      this.$emit('create-ticker', this.ticker.toUpperCase());
      this.ticker = '';
    },
